...
export default function OrderDetailPage({
  ...
  useEffect(() => {
    ...
    const fetchMessages = () => {
      apiClient
        .get<OrderMessage[]>(`/core/orders/${orderId}/messages/`)
        .then((r) => {
  ...
  const currentStageIndex = STAGES.indexOf(
    order?.status as (typeof STAGES)[number]
  );

  const handleConfirmDelivery = async () => {
    if (!order || confirming) return;
    setConfirming(true);
    const result = await apiClient.patch(`/core/orders/${orderId}/status/`, {
      status: "entregue",
    });
    if (!result.error) {
  ...
  const handleSendMessage = async () => {
    ...
    setSending(true);
    setMessageText("");
    const result = await apiClient.post<OrderMessage>(
      `/core/orders/${orderId}/messages/`,
      { content: trimmed }
    );
    if (result.data) {
      setMessages((prev) => [...prev, result.data as OrderMessage]);
    }
    setSending(false);
  };
                          ...
                          {index > 0 && (
                            <div
                              className={`flex-1 h-0.5 ${
                                reached ? "bg-orange-500" : "bg-gray-200"
                              }`}
                            />
                          )}
