...
export default function StoreProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      {showReviews && store.company_user_id && (
        <StoreReviewsModal
          companyUserId={store.company_user_id}
          storeName={store.company_name}
          rating={store.rating}
      ...
      <div className="relative h-64 bg-gradient-to-br from-orange-400 to-amber-500">
        ...
        {/* Store name overlay at bottom of banner */}
        <div className="absolute bottom-0 left-0 right-0 px-4 pb-4 pt-8">
          <div className="max-w-4xl mx-auto">
            <span className={`inline-flex items-center text-xs font-semibold px-2 py-0.5 rounded-full mb-1.5 ${store.is_open ? "bg-green-500 text-white" : "bg-red-500 text-white"}`}>
              {store.is_open ? "Aberta agora" : "Fechada"}
            </span>
            <h1 className="text-2xl font-bold text-white leading-tight drop-shadow-sm">{store.company_name}</h1>
            <p className="text-sm text-white/80 mt-0.5">{store.category}</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 -mt-4 relative z-10 p-5 mb-5">
          <div className="flex flex-wrap gap-x-5 gap-y-2 text-sm text-gray-600">
            <button
              onClick={() => store.company_user_id && setShowReviews(true)}
              disabled={!store.company_user_id}
              className="flex items-center gap-1.5 hover:text-orange-600 transition-colors group disabled:cursor-default"
              title="Ver avaliações"
            >
              <svg className="w-4 h-4 text-yellow-400 fill-yellow-400 flex-shrink-0" viewBox="0 0 20 20">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
              </svg>
              <span className="font-semibold text-gray-800 group-hover:text-orange-600">{store.rating}</span>
              <span className="text-gray-400 group-hover:text-orange-500 underline underline-offset-2 decoration-dotted">
                ({store.rating_count} avaliações)
              </span>
            </button>
        ...
        {store.items.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {store.items.map((item) => (
              <div
                key={item.id}
                className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col hover:border-orange-200 hover:shadow-md transition-all overflow-hidden"
              >
                {/* Product image — always rendered */}
                ...
                <div className="p-4 flex flex-col flex-1">
                  ...
                  <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-50">
                    <div>
                      <p className="text-xs text-gray-400 leading-none mb-0.5">Preço</p>
                      <span className="font-bold text-orange-500 text-base">R$ {Number(item.price).toFixed(2)}</span>
                    </div>
                    <button
                      onClick={() => addToCart(item.id)}
                      disabled={addingItemId === item.id}
                      className="bg-orange-500 text-white text-xs font-semibold rounded-xl px-4 py-2.5 hover:bg-orange-600 disabled:opacity-60 transition-colors flex items-center gap-1.5"
                    >
      ...
      {/* Sticky cart bar — shown after any item is added */}
      {sessionCartCount > 0 && (
          ...
          <div className="max-w-4xl mx-auto pointer-events-auto">
            ...
            >
              <span className="flex items-center gap-2">
                <span className="bg-white/20 rounded-lg w-7 h-7 flex items-center justify-center text-sm font-bold">
                  {sessionCartCount}
                </span>
                <span className="font-semibold text-sm">
                  {sessionCartCount === 1 ? "item adicionado" : "itens adicionados"}
                </span>
              </span>
