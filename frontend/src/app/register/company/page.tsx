...
import { formatCEP, formatCNPJ, formatPhone, validateCEP, validateCNPJ } from "@/utils/formatters";
...
function Step1({ data, updateData, errors, setErrors }: { 
  ...
  updateData: (updates: Partial<CompanyFormData>) => void;
  ...
  return (
    <div className="space-y-4">
      ...
      <Input
        ...
        value={data.segment}
        onChange={(e) => {
          updateData({ segment: e.target.value });
...
export default function CompanyRegisterPage() {
  ...
  const updateData = (updates: Partial<CompanyFormData>) => {
  ...
  const { currentStepIndex, step, isFirstStep, isLastStep, next, back } = useMultiStepForm<CompanyFormData>(
    steps,
    formData
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await apiClient.post("/auth/register/company/", registrationData);
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-lg p-8">
          ...
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-semibold text-gray-800">Cadastro de Empresa</h2>
